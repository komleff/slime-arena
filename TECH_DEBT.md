...
const scatter = lossActual × scatterFraction;
const gain = lossActual - scatter;
```

**Файлы:**
- `server/src/rooms/ArenaRoom.ts` — `combatSystem()`, логика PvP укуса
...
**Решение:**
- Добавить проверку `if (gameEnded) return;` в `updateOrbsVisual()`

**Файлы:**
- `client/src/main.ts` — `updateOrbsVisual()`
...
**Контекст:**
- `applyMassDelta()` сначала применяет delta, потом clamp до `[1, maxMass]`
- Это важный инвариант, который должен быть задокументирован

**Файлы:**
- `server/src/rooms/ArenaRoom.ts` — `applyMassDelta()`
...
**Контекст:**
- Сейчас GCD (`abilityGcdTick`) ставится в `abilitySystem()` до проверки успешности
- Если у игрока нет массы или кулдаун не прошёл — GCD всё равно ставится

**Решение:**
- Перенести установку `abilityGcdTick` внутрь успешной ветки `activateXxx()`

**Файлы:**
- `server/src/rooms/ArenaRoom.ts` — `abilitySystem()`
...
**Контекст:**
- `activateDash()` меняет позицию, но не проверяет границы мира
- Игрок может выйти за пределы карты

**Решение:**
- Добавить `clamp(player.x, 0, worldWidth)` после dash

**Файлы:**
- `server/src/rooms/ArenaRoom.ts` — `activateDash()`

**Статус:** ⏳ Ожидает фикса

---

### hpMult config не используется
**Задача:** Удалить или использовать параметр `hpMult`

**Контекст:**
- В `balance.json` есть `hpMult`, но он нигде не применяется
- Либо удалить, либо интегрировать в систему здоровья
