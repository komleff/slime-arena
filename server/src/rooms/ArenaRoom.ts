...
export class ArenaRoom extends Room<GameState> {
    ...
    private spawnOrb(x?: number, y?: number, mass?: number, color?: OrbColor) {
        if (this.state.orbs.size >= GAME_CONFIG.ORB_MAX_COUNT) return;
    ...
    private processCombat(attacker: Player, defender: Player, dx: number, dy: number, dist: number, now: number) {
        ...
        // Обновляем maxHP атакующего
        attacker.maxHp = getSlimeHP(attacker.mass);
    ...
    private handlePlayerDeath(player: Player, now: number) {
        ...
        for (let i = 0; i < GAME_CONFIG.DEATH_ORBS_COUNT; i++) {
            ...
            this.spawnOrb(orbX, orbY, orbMass, this.getRandomOrbColor());
            // Находим только что созданный пузырь и задаём ему скорость
            const lastOrbId = `orb_${this.orbIdCounter - 1}`;
            const orb = this.state.orbs.get(lastOrbId);
            if (orb) {
                orb.vx = vx;
                orb.vy = vy;
            }
