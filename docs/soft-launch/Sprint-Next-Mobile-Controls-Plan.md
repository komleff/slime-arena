...
Схема взаимодействия (см. раздел 4.3):
...
Ниже заданы два *Variant* (см. глоссарий) без изменения логики, только параметры (см. раздел 4.3).
...
5. Для QA обеспечить ручное переключение без пересборки (см. раздел 4.1).
