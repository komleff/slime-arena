...
**HP и масса:**
- `maxHP = масса` — системное правило
- Урон снижает HP, масса не меняется
- Потеря массы уменьшает maxHP, затем HP = min(HP, maxHP)
- Получение массы увеличивает и maxHP, и текущий HP
...
### 0.12. Изменения версии 2.5.1

**Способности классов (реализованы):**
- **Рывок (Охотник)** — 80м за 0.3с, визуал: огненный след
- **Щит (Воин)** — 3с неуязвимости, визуал: голубое свечение
- **Притяжение (Собиратель)** — 150м радиус, 1.5с, визуал: магнитное поле

**UI способностей:**
- Кнопка способности с индикатором кулдауна (SVG прогресс-круг)
- Тёмный оверлей на кулдауне, яркое свечение когда готово
- Горячие клавиши: Q, 1, 2, 3

**Технические изменения:**
- `FLAG_DASHING`, `FLAG_MAGNETIZING` — новые флаги состояния
- `abilityCooldownTick` — синхронизируется с клиентом
- Сервер не перезатирает `abilitySlot` обычными input-пакетами
- **Visual State System** — визуальное состояние отделено от серверного
- **Предиктивная экстраполяция** — `lookAheadMs = 150` компенсирует сетевую задержку
- **Velocity Integration** — движение предсказывается по скорости из снапшота
- **Catch-up коррекция** — визуальная позиция плавно догоняет целевую
...
| `angularSpeedLimitRadps` | Макс. угловая скорость | 1.396 рад/с (80°/с) |
...
| `angularStopTimeS` | Время остановки вращения | 1.0 с |
...
| `orbBitePctOfMass` | Откусывание пузыря (% от массы) | 5% |
...
|----------|----------|--------------|
| `linearDragK` | Линейное сопротивление среды | 0.1 |
| `angularDragK` | Угловое сопротивление среды | 0.1 |
...
### 2.11. HP и масса (системное правило)

**Определение:** `maxHP = массаKg`
...
**Управление:** Q или клавиши 1/2/3, также кнопка на экране.

**Рывок (Охотник):** ✅ Реализовано
- Стоимость: 3% массы
- Восстановление: 5 сек
- Дистанция: 80 м
- Длительность: 0.3 сек
- Визуал: огненный след за слаймом
- Множитель урона при столкновении: ×1.5

**Выброс:** ❌ Не реализовано
...
**Щит (Воин):** ✅ Реализовано
- Стоимость: 4% массы
- Восстановление: 8 сек
- Длительность: 3 сек
- Визуал: голубое свечение вокруг слайма
- Поглощает весь входящий урон
- Можно есть пузыри под щитом
- Атака снимает щит досрочно

**Притяжение (Собиратель):** ✅ Реализовано
- Стоимость: 1.5% массы
- Восстановление: 6 сек
- Радиус: 150 м
- Длительность: 1.5 сек
- Сила притяжения: 50 м/с к центру слайма
- Визуал: фиолетовое магнитное поле с линиями
...
- **Автовыбор:** если игрок не выбрал талант за **7 сек**, автоматически выбирается случайный

### 7.2. Быстрый выбор

**Как открывается:** нажатие на индикатор «+N» возле кнопок умений.

**Время на выбор:** 7 секунд (таймер отображается).

**Описания талантов:** краткие (2–3 слова), без развёрнутого текста.

**При бездействии:** автовыбор случайного таланта через 7 секунд.
