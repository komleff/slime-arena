# Slime Arena — Технический слепок v0.6.0

**Дата:** 21 января 2026
**Версия:** 0.6.0
**Назначение:** Контекст для внешнего ИИ при подготовке нового ТЗ
...
│   ├── src/
│   │   ├── rooms/
│   │   │   └── ArenaRoom.ts  # Игровая логика (2704 LOC)
│   │   ├── systems/          # Извлечённые системы
│   │   │   ├── combatSystem.ts
...
│   └── soft-launch/  # Архитектура (v4.2.5)
...
| Файл | Назначение | LOC |
|------|------------|-----|
| `server/src/rooms/ArenaRoom.ts` | Игровая логика | 2704 |
| `client/src/main.ts` | Клиентский код | 3790 |
| `config/balance.json` | Все параметры баланса | ~700 |
| `shared/src/config.ts` | Типизация конфига | ~200 |
| `shared/src/formulas.ts` | Игровые формулы | ~150 |
| `server/src/utils/rng.ts` | Детерминированный RNG | ~50 |
...
| Архитектура | 4.2.5 | `docs/soft-launch/SlimeArena-Architecture-v4.2.5-Part*.md` |
...
| Метрика | Значение |
|---------|----------|
| Строк TypeScript | 83,050 |
| Файлов документации | 114 |
| Коммитов | 473 |
| Pull Requests | 77 |
| Релизов | 16 |
