...
Документ описывает техническую архитектуру проекта в соответствии с `SlimeArena-GDD-v2_4.md` (v2.5.0) и инженерным ТЗ управления/физики.

---

## 0. Изменения версии 1.5.2

- **Камера и управление мышью (25.12.2025)**:
  - Камера следит за `renderState?.players.get(room.sessionId)` (сглаженная позиция)
  - Мышь привязана к позиции слайма на экране (не к центру экрана)
  - Переменные `smoothedPlayerX/Y` сохраняют сглаженную позицию для синхронизации
  - Решена проблема дёрганья камеры и инвертированного управления у краёв карты
- GDD обновлён до v2.5.0 с честной физикой орбов и параметрами управления

## 0.1. Изменения версии 1.5.1

- **U2-стиль сглаживания** — полностью переработана система клиентской интерполяции:
  - Visual State System: визуальное состояние отделено от серверного
  - Velocity Integration с весом `VELOCITY_WEIGHT=0.7`
  - Catch-up коррекция с `CATCH_UP_SPEED=10.0`
  - Упрощён `ClientNetSmoothingConfig` до единственного поля `lookAheadMs`
- Обновлена ссылка на GDD: `SlimeArena-GDD-v2_4.md` (v2.4.1).
- Раздел 10 «Клиентское отображение» полностью переписан под U2-архитектуру.

## 0.1. Изменения версии 1.5
...
| `VisualStateSystem` | U2-стиль сглаживания: visual state + velocity integration + catch-up |
...
## 10. Клиентское отображение (U2-стиль сглаживания)

Клиент получает авторитетные снапшоты 10–20 Гц и рендерит плавно, используя **U2-style predictive smoothing**.

### 10.1. Принцип U2-стиля

Ключевое отличие от классической интерполяции: **визуальное состояние (visual state)** отделено от серверного и плавно «догоняет» целевую точку.
...
### 10.2. Visual State System

Каждая сущность имеет отдельное визуальное состояние:

```typescript
type VisualEntity = {
    x: number;       // Визуальная позиция
    y: number;
    vx: number;      // Интерполированная скорость
    vy: number;
...
### 10.3. Константы сглаживания

Константы захардкожены в клиенте для оптимальности:
...
### 10.4. Алгоритм smoothStep

На каждом кадре рендера:

1. **Вычисление целевой точки** (экстраполяция на `lookAheadMs`):
   - `targetPos = serverPos + serverVel * (lookAheadMs/1000)`
   - `targetAngle = serverAngle + serverAngVel * (lookAheadMs/1000)`
...
7. **Сглаживание угла** с `ANGLE_CATCH_UP_SPEED` по кратчайшему пути

### 10.5. ClientNetSmoothingConfig (упрощённый)

В конфиге остаётся только одно поле:
...
**lookAheadMs** — экстраполяция на N миллисекунд вперёд для компенсации сетевой задержки.

### 10.6. Особенности для разных сущностей

**Игроки (slimes):**
- Полное сглаживание позиции, скорости и угла
- Угол важен для боёвки (пасть/хвост)
- Приоритет точности угла над позицией
...
5. **Устойчивость к джиттеру** — visual state продолжает движение
