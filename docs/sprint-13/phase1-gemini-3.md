...
**Объяснение:**
Требуется исправить **P1** (добавление индексов для лидербордов) для обеспечения производительности. Также необходимо верифицировать **P0** (применение изменений в `AuthService.ts`), так как без этого невозможна работа основного функционала спринта.
