...
`biteMassPercent = 2%` — процент от собственной массы, который слайм откусывает за один укус пузыря. PvP-укус описан отдельно (раздел 2.4).
...
- Размер укуса: 200 × 2% × 1.25 = 5 кг
- Пузырь 50 кг > 5 кг → откусывается 5 кг
- Талант Обжора: 5 × 1.20 = 6 кг получено
- Остаток пузыря: 45 кг (отскакивает)
...
```text
attackerGain = масса_атакующего × pvpBiteAttackerGainPct × modifiers
scatterMass  = масса_жертвы × pvpBiteScatterPct × modifiers
victimLoss   = attackerGain + scatterMass
```
...
| --- | ---: | --- |
| `pvpBiteAttackerGainPct` | 10% | Атакующий получает % от СВОЕЙ массы |
| `pvpBiteScatterPct` | 10% | Жертва теряет % СВОЕЙ массы в пузыри |
| `biteTailDamageMul` | ×1.5 | Множитель урона в хвост |

**Сводная таблица:**

| Зона попадания | Атакующий получает | Жертва теряет в пузыри | Итого урон жертве |
| --- | ---: | ---: | ---: |
| Бок | 10% своей массы | 10% своей массы | attackerGain + scatter |
| Хвост | 15% (×1.5) | 15% (×1.5) | attackerGain + scatter |

**Пример:**

Атакующий (200 кг) кусает жертву (100 кг) в бок:

- attackerGain = 200 × 10% = 20 кг
- scatterMass = 100 × 10% = 10 кг
- victimLoss = 20 + 10 = 30 кг
...
| --- | --- |
| > 10% | Тяжёлый наносит урон лёгкому (по формуле 2.4). Оба отскакивают по формуле импульса. |
...
Где:
- `restitution = 0.9` — коэффициент восстановления
...
1. Количество пузырей: `deathOrbsCount` = 4.
...
### 7.2. Пример расчёта урона PvP

Охотник (200 кг, −15% урон укусом) с талантом Острые зубы (+15%) атакует жертву (150 кг) в бок:

- attackerGain: 200 × 10% × 1.15 × 0.85 = 19.55 кг
- scatterMass: 150 × 10% = 15 кг
- victimLoss = 19.55 + 15 = 34.55 кг

### 7.3. Пример расчёта получения массы

Собиратель (+25% к размеру укуса пузырей) с талантом Обжора (+20%) укусывает пузырь:
- Размер укуса: 200 кг × 2% × 1.25 = 5 кг
- Пузырь 30 кг > 5 кг → откусывается 5 кг
- Бонус Обжоры: 5 × 1.20 = 6 кг получено
- Остаток пузыря: 25 кг
