...
| Параметр | Значение |
|---|---|
| Длительность матча `matchDurationSec` | 90 сек |
| Условие победы | Наибольшая масса по завершении матча |
| Игроков в матче | До 20 (максимум для одной комнаты) |

### 1.3. Фазы матча

| Фаза | Время | Особенности |
|---|---:|---|
| Рост | 0–30 сек | Максимальная плотность ресурсов |
| Охота | 30–60 сек | Ресурсов меньше, чаще конфликты |
| Финал | 60–90 сек | Безопасные зоны, повышенная награда за риск |
...
| Собиратель | +25% к размеру укуса пузырей | — | Притяжение | Синий |
...
| Источник | Количество |
|---|---|
| Укус пузыря | `orbBitePctOfMass` × масса игрока (10%) |
| Укус слайма в бок | `pvpBiteAttackerGainPct` (10%) своей массы атакующему + `pvpBiteScatterPct` (10%) массы жертвы в пузыри |
| Укус слайма в хвост | То же × `tailDamageMultiplier` (1.5) |
...
| Источник | Описание |
|---|---|
| PvP-укус трофей (бок-в-бок) | Атакующий получает % своей массы, жертва теряет пузыри. **Формула в [GDD-Combat.md § 2.4](GDD-Combat.md#24-укус-слайма)** |
| PvP-укус с хвостом | ×1.5 множитель урона. **Формула в [GDD-Combat.md § 2.4](GDD-Combat.md#24-укус-слайма)** |
| Укус пузыря | 2% собственной массы за бит. **Формула в [GDD-Combat.md § 2.1–2.3](GDD-Combat.md#2-механика-укуса)** |
...
|---|---|---:|---:|---:|---:|---:|---:|
| Рост | 0–30 сек | 0.3 сек | 30 | 70% | 30% | — | — |
| Охота | 30–60 сек | 0.5 сек | 25 | 30% | 45% | 25% | — |
| Финал | 60–90 сек | 1.0 сек | 15 | 10% | 30% | 40% | 20% |
...
|---|---|
| `gcdTicks` | 3 |
| Длительность | ~100 мс при 30 Гц (3 тика × ~33.3 мс) |

**Примечание:** `gcdTicks` — первичный параметр баланса, заданный в тиках симуляции. Переход на учёт через тики вместо миллисекунд обеспечивает независимость от частоты обновления и упрощает настройку баланса.
...
| Параметр | Значение |
|---|---|
| `thrustForwardN` | 27 000 |
| `thrustReverseN` | 9 000 |
| `thrustLateralN` | 11 300 |
| `turnTorqueNm` | 24 000 |
