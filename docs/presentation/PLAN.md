...
**Гипотеза:** Нетехнический специалист (гейм-дизайнер) может создать готовую к запуску игру, используя AI-агентов для всего технического стека.
...
- Мультиплеер в реальном времени (30 tick/s, Colyseus)
...
**Ключевое наблюдение Sprint 12:** Выделить одного агента для управления другими.
...
### 6.1 Синхронизация в реальном времени
...
```
┌─────────────────────────────────────────────────────────────────┐
│  НЕТЕХНИЧЕСКИЙ СПЕЦИАЛИСТ может создать готовое к запуску      │
...
| Термин | Определение |
|--------|-------------|
| **AI-агент** | Языковая модель (Claude, GPT), выполняющая задачи по инструкциям |
| **Architect** | Роль агента: планирует задачи, не пишет код |
| **Beads** | Система управления задачами, встроенная в Git |
| **CI/CD** | Непрерывная интеграция и доставка — автоматические проверки кода |
| **Colyseus** | Фреймворк для создания многопользовательских игр |
| **Детерминизм** | Свойство системы: одинаковые входные данные дают одинаковый результат |
| **God Object** | Файл с избыточной ответственностью (3000+ строк) |
| **Memory Bank** | Внешняя память проекта — файлы `.memory_bank/` с контекстом |
| **PR (Pull Request)** | Запрос на слияние изменений в основную ветку |
| **Schema** | Схема данных в Colyseus для синхронизации состояния |
| **Seeded RNG** | Генератор случайных чисел с фиксированным начальным значением |
| **tick/s** | Количество обновлений симуляции в секунду |
| **Worktree** | Изолированная рабочая директория Git для параллельной работы |
| **Zero Trust** | Методология: не доверять результату без проверки |
