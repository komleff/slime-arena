...
### Следующие шаги
- **Публикация контейнеров v0.2.2:** выдать права `packages: write` для `GITHUB_TOKEN` в настройках или добавить `CR_PAT`, затем перезапустить `Publish Docker Containers` и `Make Container Packages Public`.
