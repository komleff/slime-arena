...
- Vite HMR настраивается через переменные окружения `VITE_HMR_HOST`/`VITE_HMR_PROTOCOL` (для тестирования на мобильных устройствах по локальной сети).
- Переменные читаются из `.env.local` через `loadEnv()`.

### Фокус сессии

- **[В РАБОТЕ] PR #74: Env-based HMR config:**
  - ✅ client/vite.config.ts: `loadEnv()` + `VITE_HMR_HOST`/`VITE_HMR_PROTOCOL`
  - ✅ README.md: порт 5173 → 5174 и инструкция для LAN
