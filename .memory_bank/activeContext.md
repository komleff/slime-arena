...
-### Фокус сессии
- **Мобильный ввод:** блокировка масштабирования (maximum/minimum-scale, user-scalable=no) на время матча; защита от масштабирования жестами; защита от сенсорного ввода для всех мобильных кнопок.
...
- **Отладка mobile input:** добавлены логгирование `pointermove`/`pointerdown` в защищённой форме, `flushIdleMovement` отправляется перед умениями, pointer-capture освобождается в pointerup/cancel.
- **Защита от залипания движения:** после кликов по кнопкам умений сначала отправляется стоп-ввод (если джойстик/клавиши/мышь не активны), затем ability; pointer-capture освобождается в `pointerup/cancel`.
...
### Следующие шаги
- **Исправление бага джойстика:** Проанализирован баг с дрейфом джойстика на мобильных устройствах. Составлен и добавлен в репозиторий план рефакторинга и исправления (`docs/joystick-refactor-plan-gemini.md`). Следующий этап — реализация этого плана.
- **Релиз v0.2:** обновить описание релиза и README ссылками на контейнеры.
- **PR #27:** проверить и смержить `release/v0.2` в main.
- **Рефакторинг (Фаза 0):** типизация систем сервера, удаление дублирования mathUtils.
