...
- **[ИСПРАВЛЕНО] Залипание джойстика при активации умений:** добавлена функция `forceResetJoystickForAbility()`, которая принудительно сбрасывает джойстик и `lastSentInput` перед отправкой ability. Все три кнопки умений (1, 2, 3) теперь отправляют `moveX: 0, moveY: 0`. Добавлены `pointerdown` обработчики с `stopPropagation` для предотвращения всплытия событий.
