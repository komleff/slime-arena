...
## Последние изменения (PR #74)

### PR #74: Env-based HMR config — В РАБОТЕ

- client/vite.config.ts: HMR host/protocol теперь задаются через `VITE_HMR_HOST` и `VITE_HMR_PROTOCOL`.
- Используется `loadEnv()` для поддержки `.env.local`.
- README.md: обновлён порт клиента (5174) и добавлена инструкция для тестирования по локальной сети.

## Последние изменения (PR #61-66)

### PR #61-66: Ads Documentation Improvements — ЗАВЕРШЕНО

**Статус:** 🟢 ЗАВЕРШЕНО (merged to main)

**Цель:** Улучшить документацию в коде IAdsProvider, adsService, и провайдерах рекламы согласно требованиям стилевого руководства проекта (только русский язык, без англицизмов).

**Изменённые файлы (3):**
- `client/src/platform/IAdsProvider.ts`
  - ✅ PR #65: Добавлен русский перевод "rewarded video" → "(рекламы с вознаграждением)" в interface-level comment и showRewardedAd method
  
- `client/src/platform/YandexAdsProvider.ts`
  - ✅ PR #64: Добавлен русский перевод "preload API" → "(API предварительной загрузки)" в комментарии isAdReady()
  
- `client/src/services/adsService.ts`
  - ✅ PR #62: Добавлен `@param placement - Точка показа (placement)` для метода isReady()
  - ✅ PR #66: Добавлен `@param placement - Точка показа (placement)` для методов showAndClaim() и requestGrant()

**Дополнительные улучшения:**
- PR #61: Устранён англицизм "dev-платформы" в комментарии PlatformManager.initializeAdsProvider() → "платформы разработки"
- PR #63: Добавлен формат документации параметров: русский термин + английский в скобках

**Quality Assurance:**
- ✅ TypeScript build passes
- ✅ Все тесты проходят (npm run test)
- ✅ Детерминизм сохранён (determinism.test.js)
- ✅ Документация соответствует требованиям Copilot Instructions

**Ревью результаты:**
- ✅ Все PR одобрены (Copilot SWE Agent)
- ✅ Нет P0/P1 замечаний
- ✅ Соответствие GDD v3.3.2
- ✅ Соответствие требованиям проекта (русский язык, без жаргона)

**Коммиты:**
- Применены изменения из PR #61-66 в локальном репозитории
- Готово к push на origin

---

## Предыдущие изменения (Sprint 8)
...
- MetaServer (`server/src/meta/server.ts`) вызывал `app.listen(PORT)` без указания хоста
- В некоторых окружениях Express по умолчанию слушает на `::` (IPv6), что блокирует LAN доступ
- MatchServer уже правильно использовал `HOST = process.env.HOST || '0.0.0.0'`
...
**Ветка:** `main` (слито из `fix/v0.3.1-docker-infra`)
**Статус:** 🟢 ЗАВЕРШЕНО (Релиз опубликован)

**Цель:** Опубликовать стабильную версию v0.3.1 с поддержкой Docker и полным набором исправлений за Sprint 2-4.

**Выполненные задачи:**
- ✅ **GitHub Release:** Тег `v0.3.1` создан и опубликован.
- ✅ **Docker Registry:** Образы загружены в GHCR (ghcr.io/komleff/slime-arena-*).
- ✅ **LAN Access:** Реализовано автоопределение IP для подключения мобильных устройств в локальной сети.
- ✅ **DB Migrations:** Скрипты миграций сделаны идемпотентными (стабильный перезапуск).
- ✅ **Backup/Restore (PR #47):** Полноценная система бэкапов для PostgreSQL.
- ✅ **Services Integration:** Разделение MetaServer и MatchServer.

---

## Предыдущие изменения (v0.3.0)
...
- **Верификация:** Протестировано вручную внутри контейнера PostgreSQL, восстановление подтверждено (Stage D: 19/19).
- **Нагрузка:** CCU 500 (k6) пройден без ошибок.
...
**Ветка:** `feat/mobile-controls-flight-assist`
**Статус:** 🟢 Готово к PR

**Цель:** Улучшить отзывчивость мобильного управления, снизить инерционность поворотов и боковой занос.

**Изменённые параметры в config/balance.json:**

**Virtual Joystick (controls):**
| Параметр | Было | Стало | Эффект |
|----------|------|-------|--------|
| joystickDeadzone | 0.10 | 0.07 | Быстрее старт поворота |
| joystickSensitivity | 1.0 | 1.15 | Точнее ввод |
| joystickFollowSpeed | 0.8 | 0.95 | Меньше лага |

**Flight Assist (slimeConfigs.*.assist) — для всех 4 классов:**
| Параметр | Было | Стало | Эффект |
|----------|------|-------|--------|
| yawRateGain | 4.0 | 5.0 | Быстрее достижение угловой скорости |
| reactionTimeS | 0.15 | 0.11 | Быстрее реакция на ввод |
| angularDeadzoneRad | 0.02 | 0.012 | Меньше мёртвая зона |
| angularStopTimeS | 0.20 | 0.15 | Быстрее остановка поворота |
| angularBrakeBoostFactor | 1.5 | 1.8 | Меньше перелёт угла |
| counterAccelTimeS | 0.15 | 0.11 | Быстрее гашение заноса |
| counterAccelDirectionThresholdDeg | 30 | 22 | Раньше включается компенсация |
| overspeedDampingRate | 0.2 | 0.30 | Сильнее гашение превышения скорости |
| velocityErrorThreshold | 0.1 | 0.06 | Раньше коррекция мелких ошибок |

**World Physics (глобально):**
| Параметр | Было | Стало | Эффект |
|----------|------|-------|--------|
...
## Предыдущие изменения (UI Known Issues v0.3.0 Fixes)
